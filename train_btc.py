...
df = pd.read_sql("SELECT Data, Czas, Rate FROM btc_usd_historyczne ORDER BY Data, Czas", engine)
...
model_btc.add(LSTM(50, return_sequences=True, input_shape=(look_back, 1)))
model_btc.add(LSTM(50, return_sequences=False))
model_btc.add(Dense(25))
model_btc.add(Dense(1))
...
model_btc.fit(X_train, y_train, validation_split=0.2, batch_size=1, epochs=20, callbacks=[early_stopping])
model_btc.save('btc_usd_lstm_model.h5')
