...
df = pd.read_sql("SELECT Data, Czas, Rate FROM xlm_usd_historyczne ORDER BY Data, Czas", engine)
...
model_xlm.add(LSTM(50, return_sequences=True, input_shape=(look_back, 1)))
model_xlm.add(LSTM(50, return_sequences=False))
model_xlm.add(Dense(25))
model_xlm.add(Dense(1))

# Kompilacja i trenowanie modelu LSTM
model_xlm.compile(optimizer='adam', loss='mean_squared_error')
model_xlm.fit(X_train, y_train, batch_size=1, epochs=1)
model_xlm.save('xlm_usd_lstm_model.h5')
