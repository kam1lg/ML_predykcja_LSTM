...
df = pd.read_sql("SELECT Data, Czas, Rate FROM eth_usd_historyczne ORDER BY Data, Czas", engine)
...
model_eth.add(LSTM(50, return_sequences=True, input_shape=(look_back, 1)))
...
model_eth.fit(X_train, y_train, validation_split=0.2, batch_size=1, epochs=20, callbacks=[early_stopping])
model_eth.save('eth_usd_lstm_model.h5')
